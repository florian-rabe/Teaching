...
\[\mathll{
\FR:\iota,\; \WuV\sq \iota,\; \person\sq\iota,\; \male \sq \iota,\; \instr\sq\iota,\;\crs\sq\iota,\;\tch\sq \iota\times\iota, \\
\hc\sq\iota\times \float,\; \instr(\FR)\wedge\male(\FR), \\
\crs(\WuV)\;\tch(\FR,\WuV)\;\hc(\WuV,7.5), \\
...
\begin{compactitem}
 ...
 \item \lstinline[language=sql]|CREATE TABLE individuals (id ID, name string)|, where the id field is unique and automatically generated when inserting values
\end{compactitem}
...
% This table/figure is miles away in the PDF.
\begin{figure}\centering
\begin{tabular}{ll}
\toprule
...
Our interpretation of BOL in SQL is restricted to assertions that use only atomic expressions.
...
In particular, the table \texttt{person} misses the entry $1$ for the individual $\FR$ because the assertion {\FR\;\texttt{is-a}\;\person} is only present as a consequence
\end{example}
...
$(A,B)$ is the product type $A\times B$ with pairing operator $(x,y)$ and projection functions $\_1$ and $\_2$. $x\rA F(x)$ is $\lambda x.F(x)$.
	...
	The class \texttt{HashSet} is part of the standard library and offers function ${+}{=}$ and ${-}{=}$ to add/remove elements, \texttt{contains} to test elementhood, and \texttt{forall}, \texttt{foreach} to quantifiy/iterate over elements.
...
Note that we could easily compute the hash set \texttt{instructor.diff(male)} and add to it.
But that would not add anything to the two constituent sets.

If we think of the axioms as consistency checks, we can translate them to assertions, i.\,e., Boolean expressions that must be true.
...
\begin{compactitem}
\item It is \enquote{teaches} and \enquote{taught} instead of \enquote{teachs} and \enquote{teached},
\item It is \enquote{an instructor} instead of \enquote{a instructor},
 \item Sentences start with upper case letters.
 \item Proper nouns often have different names in the ontology than in reality, e.\,g., it should be \enquote{Florian Rabe} and \enquote{credit value} instead of \enquote{FlorianRabe} and \enquote{creditValue}.
\end{compactitem}
Moreover, the language could be polished in many places.
For example, \enquote{is a instructor and is a male} could become \enquote{is a instructor and a male} with a relatively easy special case treatment, or it could become \enquote{is a male instructor} with a more complex semantics that interprets some concepts via nouns and some via adjectives.
\end{example}
...
But for many applications (e.\,g., formal ontologies in the STEM fields), we would rather use STEM English, i.\,e., English interspersed with formulas, diagrams, and epistemic cues like \enquote{Definition}, \enquote{Theorem}, \enquote{Proof}, and even $\Box$.
...
\begin{itemize}
\item We need a lexicon to obtain inflection information and the translation tries to remedy that by appending \enquote{s} in various places.
This works in some cases but not in others.
%\item We have introduced some pattern matching and conditional rules in \Cref{fig:bolsem:eng} to alleviate the greatest awkwardnesses, but much more would be needed, which would lead to things like \enquote{Peter hass Mary} or \enquote{Peter has as childs Mary}. 
...
Say we have $P \text{ \texttt{is-a} } C$, $M \text{ \texttt{is-a} } C$, this would translate to \enquote{P is a $\sem{C}$, M is a $\sem{C}$} in BOL-pidgin, whereas in natural English we would aggregate this to \enquote{P and M are $\sem{C}$s}.
\end{itemize}
...
We will not pursue this here, but there is a special course \enquote{Logic-based Natural Language Semantics} at FAU in the Winter Semesters that covers these and related topics.
