...
%Given morphism $m:V\to W$
%\begin{blockitems}{In principle like for BOL}
%\item map $V$-expression $E$ to $W$-expression $m(E)$ by replacing every symbol reference with the expression provided by $m$
% \[y:=Y\]
%\item function symbols to function expressions
% \[f:=[x_1,\ldots,x_n]T\]
% \glec{$n$-ary function expression = term with $n$ free variables}
