...
Finally, we can use flexiformal formats like \sTeX~\cite{stex}, where we can capture more of the semantic properties.

In \Cref{fig:bolsem:narrative} we present a translation into plain in the style of the other aspect semantics translations above.
...
$i$ & is a $lex(i)$\\
$C_1 \sqcup C_2$ & $\sem{C_1}$s or $\sem{C_2}$s\\
$C_1 \sqcap C_2$ & $\sem{C_1}$s and $\sem{C_2}$s\\
%$C_1 \sqcap C_2$ & is a $\sem{C_1}$ that $\sem{C_2}$s & $C_1$ simple\\
$\forall R.C$    & $\sem{R}$s only ones who $\sem{C}$ \\
$\exists R.C$    & $\sem{R}$s one who $\sem{C}$s\\
...
$i$ & $lex(i)$\\
\end{tabular}
\caption{Interpretation Function for BOL into English}\label{fig:bolsem:narrative}
\end{figure}

\begin{figure}\centering
\begin{tabular}{l|l|l}
BOL Syntax $X$ & Narrative Semantics $\sem{X}$ in English& condition\\\hline\hline
ontology & English Document \\
$D_1,\ldots,D_n$ & $\sem{D_1},\ldots,\sem{D_n}$ \\
\hline
BOL declaration & FOL declaration \\
\kw{individual}\,$i$ & proper name\\
\kw{concept}\,$i$  & noun phrase\\
\kw{relation}\,$i$ & transitive verb \\
\kw{property}\,$i:T$ & noun phrase or adjective \\
$I\; \texttt{is-a}\; \forall R.C$ & all those that $\sem{I}$ $\sem{R}$s are $\sem{C}$\\
$I\; \texttt{is-a}\; \exists R. C$ & $\sem{I}$ has something he/she/it $\sem{R}$s that is a $\sem{C}$\\
$I\; \texttt{is-a}\; C$ & $\sem{I}$ is a $\sem{C}$\\
$I_1\; R\; I_2$ & $\sem{I_1}$ $\sem{R}$s $\sem{I_2}$\\
$I\; P\; V$ & $\sem{I}$ $\sem{P}$s $\sem{V}$\\ 
$F$ & $\sem{F}$\\
\hline
Formula & Sentence \\
$C_1 \Equiv C_2$ & $\sem{C_1}$s are exactly the $\sem{C_2}$s\\
$C_1 \Equiv C_2$ & an $\sem{C_2}$ is called an $\sem{C_1}$ & $C_1$ new constant\\
$C_1 \sqsubseteq C_2$ & $\sem{C_1}$s are $\sem{C_2}$s\\
\hline
Individual & noun phrase \\
$i$ & $lex(i)$ \\
\hline
Concept & common noun phrase \\
$i$ & $i(x)$\\
$C_1 \sqcup C_2$ & $\sem{C_1}$ or $\sem{C_2}$\\
$C_1 \sqcap C_2$ & $\sem{C_1}$ and $\sem{C_2}$\\
$C_1 \sqcap C_2$ & $\sem{C_1}$ such that $\sem{C_2}$ & $C_1$ simple\\
$\forall R.C$    & object whose $\sem{R}$ed ones are $\sem{C}$ \\
$\exists R.C$    & object that $\sem{R}$ a $\sem{C}$\\
$\dom\, R$ & all who $\sem{R}$ someone/something\\
$\rng\, R$ & all who are $\sem{R}$ed by someone/something\\
\hline
Relation & Formula with free variables $x:\iota,y:\iota$\\
$i$ & $lex(i)$\\
$R_1 \cup R_2$ & $\sem{R_1}$ or $\sem{R_2}$\\
$R_1 \cap R_2$ & $\sem{R_1}$ and $\sem{R_2}$\\
$R_1 ; R_2$ & (tricky; omitted)\\
$R^{-1}$          & is $\sem{R}$ed by\\
$R^*$       $$   & (tricky, omitted)\\
$\Delta_C$     & is itself\\
\hline
Property of type $T$ & noun phrase or adjectival phrase\\
$i$ & $lex(i)$\\
\end{tabular}
\caption{Interpretation Function for BOL into English}\label{fig:bolsem:narrative2}
\end{figure}
...
Let us have a look at some of the problems that appear.
\begin{itemize}
...
An example of this is the GF system (Grammatical Framework~\cite{gf}) which allows to specify a rich formal language \textbf{abstract syntax trees} (ASTs) together with language-specific \textbf{linearizations}, which amount to recursive functions that translate ASTs to language-specific strings.
