...
\section{Motivation: Semantics by Translation}
...
\lec{use a formal system with context-sensitive rules}
\end{frame}
...
% |- F: prop   --------------->  |- Exp(F) : Exp(prop)
% no issue if Exp(prop) = prop, i.e., l-propositions are translated to L-propositions

% But not guaranteed, totally possible (and in fact common) that Exp(prop) != prop
% Then what do we do with query |- F ?
% |- Exp(F)  does not make any sense because Exp(F) is not an L-proposition

% To use a translation with Exp(prop) != prop to answer l queries via an L-semantics, we need one more thing:
  % an operator that connects Exp(F):Exp(prop) with L-propositions
  % That is the True operator, an L-function that maps Exp(prop) -> prop
\end{frame}
