...
So far
\begin{itemize}
\item relative semantics = semantics by translation
\lec{relative to semantics of target language}
\item obvious problem: what is semantics of target language?
\item our answer so far: it's in our heads, in reality
\item obvious problem: does not work well with computers
\end{itemize}

Also need semantics that is
\begin{itemize}
\item absolute
  \lec{no reference to other languages}
\item machine-actionable
  \lec{requires an imperfect model of application domain}
\end{itemize}
\end{frame}

\begin{frame}\frametitle{Imperfect Modeling}
\begin{itemize}
...
\item Imperfect models
 \begin{itemize}
 \item language focuses on some aspects of domain
 \item vocabulary approximates domain
 \end{itemize}
\end{itemize}
...
\begin{frame}\frametitle{Querying as a Guide for Semantics}
\begin{blockitems}{Idea}
\item Very difficult to choose aspects for imperfect models
...
\begin{frame}\frametitle{Deductive Absolute Semantics}
\begin{blockitems}{Definition}
\item A set of rules that determines which propositions are theorems
 \lec{called a calculus}
\item Languages called logics
\item Systems called theorem provers
\end{blockitems}
...
\begin{blockitems}{Most logics have multiple semantics}
\item Proof theory: absolute semantics \glec{to be self-contained}
\item Model theory: relative semantics by translation to set theory \glec{common reference language}
\item Logic translation: relative by translation into standard logics, e.g., SFOL
\end{blockitems}
\end{frame}

\begin{frame}\frametitle{Concrete Absolute Semantics}
\begin{blockitems}{Definition}
\item A set of rules that determines true ground instances of a proposition
\item Languages typically called query languages
 \glec{inspired our, more general use of the word}
\item Systems called triple stores, databases
\end{blockitems}
...
\begin{frame}\frametitle{Computational Absolute Semantics}
\begin{blockitems}{Definition}
\item A set of rules that determines evaluation of expressions
\item Languages typically called programming languages
\item Systems called interpreters
\end{blockitems}

\begin{blockitems}{Examples}
\item Any interpreted language \glec{Python, bash, \ldots}
\item Machine language \lec{interpretation rules built into microchips}
\end{blockitems}

\begin{blockitems}{Many languages have multiple semantics}
\item Interpreter: absolute semantics \glec{for flexible experimentation}
\item Compiler: relative semantics \glec{for fast execution}
\end{blockitems}
\end{frame}

%\section{Equivalence of Semantics}
%
%\begin{frame}\frametitle{Problem: Conservativity}
%In general, semantics could be non-conservative:
%\begin{itemize}
%\item even if back-translation exists: how do know we get all results?
%\item Compare:
% \begin{itemize}
% \item have: $\vdash_L \delta:\sem{\Gamma}$ such that $\sem{\Gamma}\vdash_L \truelift \sem{p}[\delta]$ theorem
% \item need: $\vdash_l \gamma:\Gamma$ such that $\sem{\Gamma}\vdash_l \truelift \sem{p[\gamma]}$ theorem
% \end{itemize}
%\item Substitution theorem: for every $\gamma$, there is a $\delta=\sem{\gamma}$.
%\item Conservativity: for every $\delta$, there is a $\gamma$.
%\end{itemize}
%\end{frame}
