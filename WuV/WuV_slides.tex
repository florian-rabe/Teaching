...
\section{Primitive Types and Encoding Data}
...
\item Standard codecs for translating common types to primitive ones
\end{blockitems}

\begin{blockitems}{Codec for type $A$}
\item coding function $A$-values $\to$ primitive objects
\item partial decoding function primitive objects $\to$ $A$-values
...
\begin{blockitems}{Finite Collection Types}
 \item lists
 \item fixed-length lists (= Cartesian power, $n$-tuple)
 \item sets
 \item multisets (= bags)
 \lec{all homogeneous, e.g. $list\;A$ is type of lists over $A$}
\end{blockitems}

\begin{blockitems}{Aggregation Types}
 \item Cartesian product of some types $A\times B$
 \item disjoint union of some types $A\cup B$
 \item labeled Cartesian product (= record) $\{a: A, b: B\}$
  \glec{unordered, named projections}
 \item labeled disjoint union $a(A)|b(B)$
  \glec{uncommon, only mentioned for systematics}
\end{blockitems}
\end{frame}
