...
\item Finite aggregations: tuples, records
...
\begin{itemize}
\item an OWL ontolgoy
\item an SQL database
\end{itemize}
\end{frame}

\begin{frame}\frametitle{Two Approaches}
\begin{blockitems}{Based on \emph{untyped} ontology languages}
...
\begin{blockitems}{Based on \emph{typed} ontology languages}
...
\item Data maintained as \emph{tables}
  \glec{system = (relational) database}
\item Typical language/tool design
 \begin{itemize}
...
\begin{frame}\frametitle{Untyped concrete data}
\begin{blockitems}{Knowledge graph}
\item nodes = individuals
 \begin{itemize}
 \item identifier
 \item sets of concepts
 \item key-value sets of properties
 \end{itemize}
\item edges = relation assertions
 \begin{itemize}
 \item from subject to objects
 \item labeled with name of relation
 \end{itemize}
\end{blockitems}

\begin{blockitems}{Easy to}
...
\item query: use graph intuition to find needed data
\end{blockitems}
\end{frame}

\begin{frame}\frametitle{Typed concrete data}
\begin{blockitems}{Schema}
...
\item table, column names given by schema
\item row key = distinguished column \\
options
 \begin{itemize}
 \item preexistent characteristic column
 \item generated, e.g. UUID, incremental numbering
 \item concatenation of characteristic set of columns
 \end{itemize}
