...
\begin{center}
\begin{tabular}{l|l}
aspect & typical propositions\\
\hline
ontology language & assertions, concept equality/subsumption \\
programming language & equality for some types, booleans operators\\
...
Deductive semantics is usually undecidable, i.e., there is no algorithm that takes in $F$ returns yes or no.

Therefore, deductive querying is very difficult in general.
One has to run heuristics (theorem provers) to see if a prove of $F$ or $\neg F$ can be found.
...
Therefore, in addition to identify decidable fragments of a logic, it is desirable to identify \emph{efficiently} decidable fragments.
