...
\tb example               &  \kw{individual} \enquote{WuV}:\enquote{course}  & \kw{individual} \enquote{Wuv}, \; \enquote{WuV} \texttt{is-a} \enquote{course}\\
\midrule
...
English & proper nouns & common nouns & \enquote{$i$ is a $c$} is true \\
\bottomrule
\end{tabular}
\end{center}
...
If we want to add the concept \enquote{male}, it would be difficult to make {\FR} have both types.
...
Rather than the relations with subjects \enquote{FlorianRabe} just happening to be around as well, the type system would now force their existence and the type of the object.
...
individual "FlorianRabe": "instructor"
  ...
  "office" "11.137"
\end{lstlisting}
Now the type \enquote{instructor} forces the presence of a list of taught courses (the $^*$ is meant to indicate a list), an integer for the age, and a string for the office.
...
The words \textbf{abstract} and \textbf{concrete} do not have standard definitions for types.
