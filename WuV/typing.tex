...
Whereas copying the fields of two ADTs side-by-side is easy and already possible with flat ADTs via $\{c_1: A_1, c_2: A_2\}$ (assuming we have internal ADTs with types $A_i$), merging is a crucial feature that the language (and type system) must support.
...
But we have a problem with the complex case $(\ast)$ in Def.~\ref{def:mixinflat}: if $c:T$ occurs before $c:T=t$, we cannot simply drop the former because intermediate fields may refer to $c$ -- even before the latter field  $c:T=t$ appears.
...
A more common solution is to allow the fields to be mutually recursive in their definitions.
For example, in an OO-language you can consider two methods that call each other to be a case of two ``fields'' whose types (the method signatures) are \emph{non-recursive} (only backwards-referencing), but whose definitions are mutually recursive.
