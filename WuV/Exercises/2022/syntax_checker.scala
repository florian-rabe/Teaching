// a context-sensitive language with a syntax checker
...
// ************************ Context-Sensitive syntax checker

/* A syntax checker traverses the AST. Any such traversal consists of
   ...
   In that case, the syntax checker is a traversal that
   - takes a Vocabulary as an extra argument - that's the context that makes everything context-sensitive
   - returns Boolean
   
   A type checker additionally has to take the expected type into account for all expressions for which types are used.
*/
           ...
           case Some(d) => d match {
             case FunctionSymbolDeclaration(_, ins,out) =>
               if (ins.length != args.length) {
                 false // in practice, throw "wrong number of arguments" error
               } else {
