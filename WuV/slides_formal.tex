...
\begin{tabular}{l|l|l}
            & static & dynamic \\
\hline
introduce identifiers     & vocabularies & morphisms \\
build context-free syntax & expressions & homomorphic extension\\
restrict context-sensitively & well-formed & well-formedness preservation\\
\end{tabular}
\end{center}

\begin{itemize}
\item Vocabulary $V$ introduces list of identifiers (of various kinds)
\item $V$-expressions (of various kinds) are syntax trees using $V$-identifiers as leaves
\item Well-formed $V$ expressions satisfy additional context-sensitive typing conditions
\end{itemize}

Homomorphisms $m:V\to W$
\begin{itemize}
\item map every $V$-identifier to a $W$-expression (of the same kind)
\item map every $V$-expression to a $W$-expression (of the same kind) by homomorphic extensions
\item preserve well-formedness
\end{itemize}
\end{frame}
...
\end{itemize}
\[\Voc^l = \{V\,|\,\vdash^l V\}\]
 \glec{sometimes written $|l|$}
\[\Mor^l(V,W) = \{m\,|\,\vdash^l m:V\to W\}\]
 \glec{sometimes written $l(V,W)$}
\[\Exp^l_V(E) = \{e\,|\,\vdash^l_V e:E\} \tb \Exp^l_V = \bigcup_E \Exp^l_V(E)\]
\[\Exp^l_m(e):\Exp^l_V \to \Exp^l_W, \tb\text{defined by homomorphic extension}\]
\end{frame}
