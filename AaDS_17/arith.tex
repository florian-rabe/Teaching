...
\begin{acode}
\afun[\N]{fib}{n:\N}{
  \aifelse{x\leq 1}{x}{fib(n-1)+fib(n-2)}
}
\end{acode}
...
\subsection{Linear Algorithms}\label{sec:ad:fib:linear}
...
%% exercise: implement all of the above; find smallest n for which inexact algo wrong (for me in python: 71), highest computable with fibExp (for me: around 35)
...
Addition: For of $x,y\in \Z^{mn}$, we define $x+y\in\Z^{mn}$ by
\[(x+y)_{ij}=x_{ij}+y_{ij}\]

Multiplication: For $x\in \Z^{lm}$ and $y\in \Z^{mn}$, we define $x\cdot y\in\Z^{ln}$ by
